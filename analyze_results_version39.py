...
q17=0.7
id_scenario = 'hes%i_fA%i_fV%i_q17%i.txt' % (int(source2.hesitancy*100),int(source2.f_A*100),int(source2.f_V*100),int(q17*100))
id_scenario = 'hes30_fA75_fV50_q1770'
infix+='_'+id_scenario


row_index = 1
param = np.array(np.array(source.data)[row_index,5:5+6],dtype=np.float64)
beta = source2.parm_to_beta(param[:2])
q = source2.parm_to_q(param[2:4])
midc = param[4]
exponent = param[5]**2

#        source2.get_initial_values.recompile()
#        source.fitfunc_short.recompile()
...
C.to_excel('v%s_summary_all_relative_comparison.xlsx' % infix)
...
for i in range(nr_scenarios):        
    ...
    for j in range(len(types_legend)):
        if j==2 or j==4:
            continue
        counter+=1
        ax.plot([deaths_optimal[i,j]],[cases_optimal[i,j]],'X',color=colors[counter],label=types_legend[j])  
...
for i in range(nr_scenarios):
    ...
    result_CDC = source.fitfunc_short(np.array(CDC_allocation),initial_values,beta,q,midc,exponent,source2.f_A,source2.f_V,source2.sigma,source2.delta)
    deaths_per_age = np.array(result_CDC[:4])
    ax[0,0].bar(range(4),deaths_per_age/sum(deaths_per_age),color='#2F2F2F',label='CDC')
    ax[counter//2,counter%2].text(1.5,0.5,'CDC',color=colors[counter],va='center',ha='center')
    for k in range(4):
        ax[counter//2,counter%2].text(k,0.06,str(round((100*deaths_per_age/sum(deaths_per_age))[k],2))+'%',va='center',ha='center',color='white' if k>0 else '#2f2f2F')
    for j in range(len(types)):
        if j==2 or j==4:
            continue
        counter+=1
        result = source.fitfunc_short(np.array(optimal_allocationss[i][j]),initial_values,beta,q,midc,exponent,source2.f_A,source2.f_V,source2.sigma,source2.delta)
        deaths_per_age = np.array(result[:4])
        ax[counter//2,counter%2].bar(range(4),deaths_per_age/sum(deaths_per_age),color='#2F2F2F',label=types_legend[j])
...
for which_group in [0,1,7,14]:
    ...
    #    ax.set_xlabel('total death count')
    #    ax.set_ylabel('total cases')
    #    plt.savefig('v%s_random1000points_stratified_by_group%i_scenario%i%s' % (infix,j+1,which_group+1,ending),bbox_inches = "tight")
    #    
...
for j in range(nr_scenarios):
    ...
    for k,which_group in enumerate(range(17)):
        ...
        ax[(k+3)//4,(k+3)%4].text(740000,42.7*1e6,'Group %i' % (which_group+1),va='center',ha='center')
        if k==-3:
    ...
    for k,(which_group,name) in enumerate(zip([0,1,7],['children','healthcare workers\nwithout comorbidities','16-64 year olds\nwithout comorbidities'])):
        ...
        ax[k].set_yticklabels([int(el//1000000) for el in ax[k].get_yticks()])
#        ax[k].text(750000,43.4*1e6,'Group %i' % (which_group+1),va='center',ha='center')
        ax[k].text(740000,43.2*1e6,name,va='top',ha='center')
        ax[k].set_ylim([39*1e6,43.2*1e6])
...
plt.savefig('v%s_sigma_delta_fv_deaths_base_scenario%i_ve%s%s' % (infix,1,ve,ending),bbox_inches = "tight")
...
plt.savefig('v%s_sigma_delta_fv_cases_base_scenario%i_ve%s%s' % (infix,1,ve,ending),bbox_inches = "tight")
...
cbar.set_ticklabels([str(int(el//1000)) for el in cbar.get_ticks()])
cbar.set_label('deaths [thousands]')
plt.savefig('v%s_sigma_delta_fv_deaths_scenario%i%s' % (infix,1,ending),bbox_inches = "tight")
...
cbar.set_ticklabels([str(int(el//1000)) for el in cbar.get_ticks()])
cbar.set_label('deaths [thousands]')
plt.savefig('v%s_sigma_delta_fv_flipped_scenario%i%s' % (infix,1,ending),bbox_inches = "tight")
...
ax.legend(loc='best')
plt.savefig('v%s_dynamics_CDC_scenario%i_linear%s' % (infix,scen,ending),bbox_inches = "tight")
...
plt.savefig('v%s_dynamics_CDC_scenario%i_log%s' % (infix,scen,ending),bbox_inches = "tight")
...
plt.savefig('v%s_fit%s' % (infix,ending),bbox_inches = "tight")
...
pd.DataFrame(np.c_[real_d,model_d,real_d-model_d,real_c,model_c,real_c-model_c]).to_excel('v%s_fit%s' % (infix,'.xlsx'))
...
f,ax = plt.subplots(figsize=(4,3))
last_day = len(source.cumcases[source.index_2020_10_13:])-62
cum_deaths_real = source.cumdeaths[source.index_2020_10_13:]
cum_deaths_model = sol_by_compartment[:,17][::2][:len(cum_deaths_real)]
daily_deaths_real = source.filtered_daily_deaths[source.index_2020_10_13:]#cum_deaths_real[1:]-cum_deaths_real[:-1]
...
cum_cases_real = source.cumcases[source.index_2020_10_13:]
...
daily_cases_real = source.filtered_daily_cases[source.index_2020_10_13:]#cum_deaths_real[1:]-cum_deaths_real[:-1]
...
plt.savefig('v%s_fit_daily%s' % (infix,ending),bbox_inches = "tight")
...
#ax.spines['right'].set_visible(False)
#ax.spines['top'].set_visible(False)
#plt.savefig('v%s_r%s' % (infix,ending),bbox_inches = "tight")
