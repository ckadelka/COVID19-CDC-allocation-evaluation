...
if option==1:
    ...
    data = pd.read_csv('v%s_GA_output_fafv.csv' % version)
    ...
    #
        ...
        for f_A_index in range(3):
            for f_V_index in range(3):                                
                ...
                source2.f_A = np.array([0.75,0.25,1])[f_A_index]
                source2.f_V = np.array([0.5,0,1])[f_V_index]
 
                param = np.array(np.array(data)[row_index,6:6+4],dtype=np.float64)
                ...
                for sigma_index in range(3):
                    if sigma_index == 0:
                        source2.sigma = 0.9*np.ones(17,dtype=np.float64)
                        source2.delta = 0*np.ones(17,dtype=np.float64)
                    elif sigma_index == 1:
                        source2.sigma = 0.7*np.ones(17,dtype=np.float64)
                        source2.delta = 2/3*np.ones(17,dtype=np.float64)
                    elif sigma_index == 2:
                        source2.sigma = 0*np.ones(17,dtype=np.float64)
                        source2.delta = 0.9*np.ones(17,dtype=np.float64)
                    ...
                    initial_values=source2.get_initial_values(source2.Nsize,source2.mu_A,source2.mu_C,source2.mu_Q,q,hesitancy=source2.hesitancy)    
                    for ID in range(SLURM_ID*nsim,(SLURM_ID+1)*nsim):
                        vacopt = np.array(get_i1_to_i17_which_satisfy_all_constraints(ID))
                        
                        res_from_run = fitfunc_short(np.array(vacopt),initial_values,beta,q,midc,exponent,source2.f_A,source2.f_V,source2.sigma,source2.delta)
                        
                        total_deaths.append(sum(res_from_run[:4]))
                        total_cases.append(sum(res_from_run[4:8]))
                        ylls.append(np.dot(res_from_run[:4],yll_4agegp))
                    ...
                    argmins_deaths.append(vacopts_ids[np.argmin(total_deaths)])
                    best_deaths.append(np.min(total_deaths))
                    argmins_ylls.append(vacopts_ids[np.argmin(ylls)])
                    best_ylls.append(np.min(ylls))
                    argmins_cases.append(vacopts_ids[np.argmin(total_cases)])
                    best_cases.append(np.min(total_cases))
                    
        f = open(output_folder+'condensed_output_nsim%i_SLURM_ID%i_hes%i.txt' % (nsim,SLURM_ID,int(source2.hesitancy*100)) ,'w')
        f.write('filename\t'+filename+'\n')
        f.write('SLURM_ID\t'+str(SLURM_ID)+'\n')
        f.write('nsim\t'+str(nsim)+'\n')
        f.write('time in seconds\t'+str(int(time.time()-TIME))+'\n')
        f.write('argmins_deaths\t'+'\t'.join(list(map(str,argmins_deaths)))+'\n')
        f.write('best_deaths\t'+'\t'.join(list(map(str,best_deaths)))+'\n')
        f.write('argmins_ylls\t'+'\t'.join(list(map(str,argmins_ylls)))+'\n')
        f.write('best_ylls\t'+'\t'.join(list(map(str,best_ylls)))+'\n')
        f.write('argmins_cases\t'+'\t'.join(list(map(str,argmins_cases)))+'\n')
        f.write('best_cases\t'+'\t'.join(list(map(str,best_cases)))+'\n')
        f.close()
        print(time.time()-TIME)
